...
Matrix<ValueT>::unary_map(const UnaryMatrixOperator &op) const
{
    ...
    const auto radius_x = op.radius_x;
    const auto radius_y = op.radius_y;
    const auto width = 2 * radius_x + 1;
    const auto height = 2 * radius_y + 1;

    const auto start_y = radius_y;
    const auto end_y = n_rows - radius_y;
    const auto start_x = radius_x;
    const auto end_x = n_cols - radius_x;


    for (uint y = start_y; y < end_y; ++y) {
        for (uint x = start_x; x < end_x; ++x) {
            auto neighbourhood = submatrix(y - radius_y, x - radius_x, height, width);
            tmp(y, x) = op(neighbourhood);
        }
    }
    return tmp;
}
...
Matrix<ValueT> binary_map(const BinaryMatrixOperator& op, const Matrix<ValueT>& m1, const Matrix<ValueT>& m2)
{
    if (m1.n_cols * m1.n_rows * m2.n_cols * m2.n_rows == 0)
        return Matrix<ValueT>(0, 0);
    if (m1.n_cols != m2.n_cols || m1.n_rows != m2.n_rows)
        throw std::string("binary matrix operator: matrices of different size");
        
    Matrix<ValueT> tmp(m1.n_rows, m1.n_cols);
    ...
    const auto start_y = radius_y;
    const auto end_y = m1.n_rows - radius_y;
    const auto start_x = radius_x;
    const auto end_x = m1.n_cols - radius_x;


    for (uint y = start_y; y < end_y; ++y) {
        for (uint x = start_x; x < end_x; ++x) {
            auto neighbourhood1 = m1.submatrix(y - radius_y, x - radius_x, height, width);
            auto neighbourhood2 = m2.submatrix(y - radius_y, x - radius_x, height, width);
            tmp(y, x) = op(neighbourhood1, neighbourhood2);
        }
    }
    return tmp;
}
